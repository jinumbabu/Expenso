...
class SmsAgentResult {
  ...
  @override
  String toString() {
    return 'SmsAgentResult(amount: $amount, merchant: $merchant, type: $transactionType, account: $account, ref: $referenceId, confidence: $confidence)';
  }
}
...
class SmsAgent {
  ...
  // Define templates for the 10 banks
  static final List<BankTemplate> _bankTemplates = [
    ...
    BankTemplate(
      ...
      debitPatterns: [
        ...
        RegExp(r'spent\s*on\s*credit\s*card\s*(?:ending)?\s*(\d{4})\s*at\s*(.+?)\s*on', caseSensitive: false),
      ],
      creditPatterns: [
  ...
  static final RegExp _genericAmountRegExp = RegExp(r'(?:rs\.?|inr|₹)\s*([\d,]+\.?\d*)', caseSensitive: false);
  static final RegExp _genericAccountRegExp = RegExp(r'\b(?:a/c|card|ending|xx)\s*(\d{4})\b', caseSensitive: false);
  static final RegExp _genericUpiRefRegExp = RegExp(r'\b(?:upi\s*ref|ref\s*no|txn|vpa|ref)\s*:?\s*([a-z0-9]+)\b', caseSensitive: false);

  static final RegExp _merchantRegExp = RegExp(
    r'\b(?:at|to|paid\s+to|spent\s+at)\s+([a-z0-9\s\.\-_&@]+?)\s+(?:on|by|using|ref|vpa|rs\.?|inr|₹|\.|\bfor\b|\bwith\b|$)', 
    caseSensitive: false
  );
  ...
  Future<SmsAgentResult?> processSms(String body, DateTime smsDateTime) async {
    ...
    if (amount == null) {
      final amtMatch = _genericAmountRegExp.firstMatch(body);
      if (amtMatch != null) {
    ...
    // 5. Log the Decision in the Database AgentLogs table
    try {
      await _db.agentLogDao.insertLog(
        AgentLog(
          ...
          decisionDescription: 'Parsed SMS successfully. Bank: $matchedBank, Amount: ₹$amount, Type: $type, Account: $accountName',
  ...
  String _cleanMerchantName(String raw) {
    ...
    final indexRef = cleaned.toLowerCase().indexOf(RegExp(r'\b(?:ref|ref\s+no|upi|vpa|txn)\b'));
    if (indexRef != -1) {
